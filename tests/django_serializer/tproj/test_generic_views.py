...
class TestCreateApiView:
    ...
    def test_success(self, json_client):
        resp = json_client.post("/create", json={"f": 1.1, "i": 1, "nullable": "null"})
...
class TestUpdateApiView:
    ...
    def test_without_perm(self, json_client, some_model_without_perm):
        resp = json_client.post("/update", json={"id": some_model_without_perm.pk})
        assert resp.status_code == 403
        document = resp.json()
        assert document == {"data": {}, "message": "Forbidden", "status": "forbidden"}
    ...
    def test_success(self, json_client, some_model):
        resp = json_client.post(
            "/update", json={"id": some_model.pk, "f": 2, "i": 2, "nullable": "new"}
        )
        assert resp.status_code == 200
        document = resp.json()
        assert document == {
...
class TestDeleteApiView:
    ...
    def test_without_perm(self, json_client, some_model_without_perm):
        resp = json_client.post("/delete", json={"id": some_model_without_perm.pk})
        assert resp.status_code == 403
        document = resp.json()
        assert document == {"data": {}, "message": "Forbidden", "status": "forbidden"}
