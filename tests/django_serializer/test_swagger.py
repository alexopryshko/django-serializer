...
from tests.tproj.generic_views import (
    SomeModelGetView,
    SomeModelCreateView,
    SomeModelUpdateView,
    SomeModelDeleteView,
...
class TestSwagger:
    def test_form2schema(self):
        ...
        assert str(schema._declared_fields["i"]) == str(f.Integer(required=True))
        assert str(schema._declared_fields["f"]) == str(f.Float(required=True))
        assert str(schema._declared_fields["dec"]) == str(f.Decimal(required=True))

        assert str(schema._declared_fields["b"]) == str(f.Boolean(required=True))
        ...
        assert str(schema._declared_fields["dt"]) == str(f.DateTime(required=True))

        assert str(schema._declared_fields["i_not_required"]) == str(
            f.Integer(required=False)
        )
        assert str(schema._declared_fields["mc"]) == str(StrList(required=True))
        assert str(schema._declared_fields["mmc"]) == str(IntList(required=True))

    def test_swagger(self, client):
        resp = client.get("/swagger.json")
        json = resp.json()
        assert set((json["components"]["schemas"].keys())) == {
            "ListSomeModelSerializer",
            "SomeModelSerializer",
            "TestSerializer",
            "BadRequest",
            "NotFound",
        }
        assert set((json["paths"].keys())) == {
            "/500",
            "/create",
            "/delete",
            "/get",
    ...
    def test_get_view(self, client):
        ...
        assert (
            path_responses["200"]["content"]["application/json"]["schema"]["$ref"]
            == "#/components/schemas/SomeModelSerializer"
        )
    ...
    def test_create_view(self, client):
        ...
        path_responses = path["responses"]
        path_tags = path["tags"]
        assert sorted(list(path_body["properties"].keys())) == ["f", "i", "nullable"]
    ...
    def test_update_view(self, client):
        ...
        assert sorted(list(path_body["properties"].keys())) == [
            "f",
            "i",
            "id",
            "nullable",
    ...
    def test_paginate_list_view(self, client):
        ...
        assert (
            path_responses["200"]["content"]["application/json"]["schema"]["$ref"]
            == "#/components/schemas/ListSomeModelSerializer"
        )
