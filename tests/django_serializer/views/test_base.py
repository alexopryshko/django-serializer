...
class TestMeta:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "summary",
                1,
                ["`summary` has incorrect type, " "should be `<class 'str'>`"],
            ),
            ("summary", "s", None),
            (
                "description",
                1,
                ["`description` has incorrect type, " "should be `<class 'str'>`"],
            ),
            ("description", "s", None),
            (
                "query_form",
            ...
            (
                "serializer_many",
                "1",
                ["`serializer_many` has incorrect type, " "should be `<class 'bool'>`"],
            ),
            ("serializer_many", True, None),
            (
                "errors",
                [Exception, 1],
                ["`errors` item has incorrect type, " "should be subtype of HttpError"],
            ),
    ...
    def test_var(self, field, value, expected_errors):
        errors = self._create_meta(
            {"tags": ["tags"], "method": HttpMethod.GET, field: value}
        )
        assert errors == expected_errors
